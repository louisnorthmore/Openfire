 ...
 *
 * This implementation makes use of an external Validator class that is responsible for verifying user-provided
 * credentials.
 *
...
public class OAuthBearerSaslServer implements SaslServer
{
    ...
    public OAuthBearerSaslServer(BearerTokenValidator validator)
    {
        if ( validator == null ) {
    ...
    protected byte[] processInitialClientResponse( byte[] response ) throws SaslException
    {
        ...
        try
        {
            final ValidationResult validationResult = validator.validate( auth, host, port, mthd, path, post, qs );
            if ( validationResult.isValid() )
            {
                authorizationID = validationResult.getAuthorizedUser() + "@" + XMPPServer.getInstance().getServerInfo().getXMPPDomain();
            ...
            else
            {
                ...
                final ServerErrorResponse error = new ServerErrorResponse( validationResult.getErrorMessage() );
